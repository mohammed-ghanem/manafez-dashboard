...
export const ActLogin = createAsyncThunk(
  ...
  async (payload: LoginPayload, { rejectWithValue }) => {
    try {
      ...
      // 3) IMPORTANT: Fetch the full profile after login
      try {
        // We need to fetch profile separately because login might not return full user data
        const profileResp = await api.get("/auth/profile");
        const user = profileResp.data?.data || profileResp.data;
        ...
        // If profile fetch fails, use the user from login response (might be incomplete)
...
//       // Store in cookies (redux-persist replacement)
//       Cookies.set("access_token", token, { expires: 7 });
//       Cookies.set("user", JSON.stringify(user), { expires: 7 });
...
// ... rest of your thunks remain mostly the same
